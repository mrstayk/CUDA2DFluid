...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
								D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float* color)
{
	bool result;
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// Setup the description of the color dynamic constant buffer that is in the pixel shader.
...
bool TextureShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float* color)
{
	HRESULT result;
    D3D11_MAPPED_SUBRESOURCE mappedResource;
	MatrixBufferType* dataPtr;
	ColorBufferType* dataPtr1;
	...
	// Get a pointer to the data in the constant buffer.
	dataPtr1 = (ColorBufferType*)mappedResource.pData;

	// Copy the water data into the constant buffer.
	dataPtr1->red = color[0];
	dataPtr1->green = color[1];
	dataPtr1->blue = color[2];
	...
	//deviceContext->PSSetShaderResources(2, 1, &density);
	return true;
}
